...
var redux_mock_store_1 = __webpack_require__(8);
var redux_thunk_1 = __webpack_require__(9);
var actions = __webpack_require__(7);
var middleware = [redux_thunk_1.default];
var mockStore = redux_mock_store_1.default(middleware);
describe("fetchCompaniesAsync", function () {
    var store = void 0;
    beforeAll(function () {
        store = mockStore({
            searchResults: []
    ...
    it("should fetch the matching companies\n    and dispatch SearchResults\n    and return true after a successful call", function () {
        var actionCreator = actions.fetchCompaniesAsync;
        var searchPhrase = "msf";
        var isSymbol = true;
        var res = true;
        function mockResponse(status, response) {
            var blobbedRes = new Blob([JSON.stringify(response)]);
        ...
        window.fetch = jest.fn().mockImplementation(function () {
            return Promise.resolve(mockResponse(200, res));
        });
        var expectedActions = [{
            type: actions.SEARCH,
            results: Promise.resolve({})
        }];
        return store.dispatch(actionCreator(searchPhrase, isSymbol)).then(function (status) {
            console.log(status);
...
describe("addWatchAsync", function () {
    var store = void 0;
    beforeAll(function () {
        store = mockStore({
            showModal: true
    ...
    it("should dispatch ToggleModalDisplay \n    and return true after a successful call", function () {
        var actionCreator = actions.addWatchAsync;
        var symbol = "msft";
        var price = 1.99;
        var phone = "+15555555555";
        var mockBody = new Blob([JSON.stringify({
            symbol: symbol,
            price: price,
            phone: phone
        })]);
        ...
        return store.dispatch(actions.addWatchAsync(symbol, price, phone)).then(function (status) {
            expect(status).toBe(true);
            expect(store.getActions()).toEqual(expectedActions);
        });
    });
});
describe("addGraphAsync", function () {
    var store = void 0;
    beforeAll(function () {
        store = mockStore({
            graphs: []
    ...
    it("should dispatch addGraph", function () {
        var company = {
            symbol: "msft",
            name: "Microsoft"
        };
        ...
        return store.dispatch(actions.addGraphAsync(company)).then(function () {
            expect(store.getActions()).toEqual(expectedActions);
        });
    });
});
